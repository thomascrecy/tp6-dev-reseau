...
async def handle_client_msg(reader, writer):
    while True:
        ...
        if data == b'':
            if addr in CLIENTS:
                pseudo = CLIENTS[addr].get("pseudo", f"{addr[0]}:{addr[1]}")
                print(f"Client {pseudo} disconnected.")
                del CLIENTS[addr]

                for client in CLIENTS.keys():
                    clientData = CLIENTS[client]
                    clientData["w"].write(f"Annonce : {pseudo} a quitté la chatroom\n".encode())
                    await clientData["w"].drain()
            break
